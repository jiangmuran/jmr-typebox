...
| Tab 结构 | **7 个顶层 tab**:编辑器 / TXT / 图片 / 转换 / 媒体 / 工具箱 / Python。TXT 独立保留(快速建 txt,尤其移动端);Python 独立成 tab 做大做强。可在设置里隐藏不常用 tab。 |
...
| `/python` | Python 演练场(Pyodide,懒加载,缓存,富输出) | Python |
| `/python/*` | 示例 / 分享片段 | Python |
| `/settings` *(或抽屉)* | 设置/自定义 | — |
...
2. **7 个克制的分组 tab** — 顶层只暴露大类(编辑器/TXT/图片/转换/媒体/工具箱/Python)。
...
- (Python 解释器已升级为独立 tab,见 §9.8)
- 与 §11 右键插件**共享同一套纯函数**,避免重复实现(右键=就地选区、工具箱=独立页、⌘K=搜索,三入口一套能力)。

### 9.8 Python 演练场(`/python`,独立 tab,做大做强)

浏览器内完整 Python(**Pyodide**,WASM 沙箱,无后端),定位为可日常使用的代码演练场:
- **代码编辑器**:语法高亮/缩进(CodeMirror,本地),多片段 + 示例库(引导探索)。
- **运行 + 富输出/动态渲染**:stdout/stderr 控制台 + **内联渲染** matplotlib 图、pandas 表、HTML/SVG、图片、返回值。
- **包安装**:`micropip` 装纯 Python 包;拉取 PyPI 属网络操作 → 优先经后端代理(可选,带「i」),否则直连并提示。
- **页内预览(实验)**:运行生成 HTML / 简单 WSGI·Flask 应用 → 经 service worker 虚拟服务在 iframe 内**页内预览**(无法开真实端口,做等价模拟,标注实验)。
- **缓存**:Pyodide 运行时与已装 wheel 用 Cache API/IndexedDB 持久缓存 → 二次秒开、可离线。
- **保存/分享**:片段存 localStorage,可导出;`/python/*` 承载示例与分享片段。
- 全程懒加载 + 体积提示;注册进命令面板。
...
- **持久缓存**:Pyodide 运行时/wheel、ffmpeg.wasm 核心、主题字体用 Cache API/IndexedDB 缓存 → 二次秒开、可离线。
...
**开始面板 `StartPanel`**(有诱惑力、引导用户继续逛):
- **出现时机**:首次访问;**编辑器空状态**(无内容时显示,开始输入即隐去 → 兼顾"极简默认"与"诱导探索");随时经 logo / ⌘K「Home」唤起。
- **内容**:动画黑白 logo + 一句话定位;**功能卡片网格**(从 `commandRegistry` 渲染:编辑器/图片/转换/媒体/工具箱/Python,各带图标 + 一句话 + 一键直达 + 精致悬停动效);**快捷动作**(新建 / 打开 / **智能粘贴**——粘贴图片→图片工具、粘贴文本→编辑器/工具箱);**最近文件 / 最近工具**;`⌘K 搜索一切` 提示。
- **审美**:克制精致、留白充分、不花哨。

**新手引导**(首次,简短、可跳过、可重看):这是什么(本地优先 / 隐私 / 离线)→ 能搬哪些日常工作流(写作→多格式导出;压图/转格式/水印/标注;音频转换;PDF↔MD↔Word;文本编解码/加密;Python)→ 如何开始(拖 / 点 / 粘)+ 教 `⌘K` → 落到开始面板/编辑器。
...
- **专业 README.md**(双语):
  ...
  - 特性总览(按 7 tab + 命令面板 + 右键插件 + 主题 + 离线/隐私)
  - 截图/GIF
...
- **开始面板(Home / 编辑器空状态)+ 新手引导骨架**:从 commandRegistry 渲染功能卡片,诱导探索。
- 扩展新手引导。
...
- **Agent G — 工具箱**:`/tools/*`(字数统计、Base64/Hex/URL/HTML 编解码、AES/哈希/HMAC、JSON、JWT)+ 工具网格首页;与右键插件共享纯函数;注册进命令面板;SEO 页。
- **Agent H — Python 演练场**:`/python` 独立 tab(Pyodide + CodeMirror + 富输出/动态渲染 + micropip 装包 + 页内预览(实验)+ Cache/IndexedDB 缓存 + 示例库);懒加载;注册进命令面板;SEO 页。
- **Agent F — 艺术级 UI 打磨**:在 A–E、G、H 落地后做整合性视觉/交互终审(frontend-design + 截图校验)。建议作为收尾串行步骤而非并行。
