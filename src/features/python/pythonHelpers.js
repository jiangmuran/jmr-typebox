...
export const EXAMPLE_PROJECTS = [
  ...
  {
    id: 'input',
    title: { en: 'Interactive input()', zh: '交互式 input()' },
    needsNetwork: false,
    entry: 'main.py',
    files: {
      'main.py': [
        '# input() is interactive. Type lines in the console box below (press Enter to queue',
        '# each one), then Run — or just Run and answer the prompt that pops up.',
        'name = input("What is your name? ")',
        'print(f"Hello, {name}!")',
        '',
        'total = 0',
        'while True:',
        '    raw = input("Add a number (blank to stop): ")',
        '    if raw.strip() == "":',
        '        break',
        '    try:',
        '        total += float(raw)',
  ...
  {
    id: 'webapp',
    title: { en: 'Web app preview (experimental)', zh: '网页应用预览(实验性)' },
    needsNetwork: false,
    entry: 'main.py',
    files: {
      'main.py': [
        '# EXPERIMENTAL: define a WSGI app called `app` and a preview pane appears in the output',
        '# panel. A synthetic request is routed to `app` in-interpreter (no real socket). Type a',
        '# path like /about and press Go. This demo uses a tiny hand-rolled WSGI app (stdlib only,',
        '# offline); a Flask `app` works too if you install flask via micropip.',
        'from html import escape',
        '',
        'PAGES = {',
        '    "/": "<h1>Home</h1><p>Welcome! Try <a href=\\"/about\\">/about</a>.</p>",',
        '    "/about": "<h1>About</h1><p>Served by a WSGI app inside Pyodide.</p>",',
        '}',
        '',
        '',
        ...
        '        return [f"<h1>404</h1><p>No page at {escape(path)}</p>".encode()]',
        '    page = f"<!doctype html><meta charset=utf-8><body style=\\"font-family:sans-serif\\">{body}</body>"',
        '    start_response("200 OK", [("Content-Type", "text/html; charset=utf-8")])',
        '    return [page.encode()]',
        '',
        '',
        'print("WSGI app ready — see the Web preview pane in the output panel.")',
        '',
      ].join('\n'),
    },
  },
  {
    id: 'fastapi',
    title: { en: 'FastAPI app (ASGI · micropip)', zh: 'FastAPI 应用(ASGI · micropip)' },
    needsNetwork: true,
    entry: 'main.py',
    files: {
      'main.py': [
        '# FastAPI / ASGI preview. fastapi ships with Pyodide, so it loads from the CDN on first',
        '# import (no micropip needed) — just press Run. A preview pane appears: a synthetic request',
        '# is driven through the ASGI app in-interpreter — no real socket. Try paths like /,',
        '# /hello/Ada, and /items?limit=2 (JSON responses are shown pretty-printed).',
        ...
        '',
        '',
        '# Handlers are async def — on Pyodide (single-threaded, no worker pool) async routes run',
        '# directly on the event loop, which is the reliable path for the in-page preview.',
        ...
        '      <p>Running on ASGI inside Pyodide. Try',
        '      <a href="/hello/Ada">/hello/Ada</a> or',
        '      <a href="/items?limit=2">/items?limit=2</a>.</p>',
        '    </body>',
        '    """',
        '',
        '',
        ...
        '',
        '@app.get("/items")',
        'async def items(limit: int = 3):',
        '    data = [{"id": i, "name": f"item-{i}"} for i in range(1, limit + 1)]',
        '    return {"count": len(data), "items": data}',
        '',
        '',
        'print("FastAPI app ready — see the Web preview pane (install fastapi first).")',
        '',
      ].join('\n'),
    },
  },
  {
    id: 'network',
    title: { en: 'Fetch a public API (CORS proxy)', zh: '请求公共 API(跨域代理)' },
    needsNetwork: true,
    entry: 'main.py',
    files: {
      'main.py': [
        '# Cross-origin HTTP from Python. Browsers normally block these requests (CORS), so the IDE',
        '# transparently routes urllib / requests through the site\'s backend proxy. Just call the',
        '# public API as usual — no extra setup. (Requires the optional backend; see the Network',
        '# note under the editor.)',
        'import json',
        'from urllib.request import urlopen',
        '',
        '# A tiny, CORS-unfriendly public API. The request is proxied automatically.',
        ...
        '',
        '# `requests` works too once installed via micropip (add "requests" above):',
        '#   import requests',
        '#   r = requests.get("https://api.github.com/repos/pyodide/pyodide")',
