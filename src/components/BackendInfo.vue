...
// backend is optional + open source. The popover is teleported to <body> so it never
// gets clipped by (or hidden behind) a scrolling container like the Settings drawer.
...
<template>
  <span class="bi">
    <button ref="triggerEl" class="bi-btn" type="button" :aria-label="t('backend.notice')" @click.stop="toggle">i</button>
    <Teleport to="body">
      <span v-if="open" class="bi-away" @click="open = false"></span>
      <Transition name="bi-fade">
        <span v-if="open" class="bi-pop" :style="popStyle">
          {{ t('backend.notice') }}
...
.bi-pop {
  position: fixed; z-index: 600; width: 220px; max-width: 88vw;
